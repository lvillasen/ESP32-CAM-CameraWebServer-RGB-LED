...
// LV 9/Dic/23
// Configure pins for LED RGB
const int pinRed = 15;  // GPIO15 for  LED red
const int pinGreen =13; // GPIO13 for  LED green
const int  pinBlue = 12; // GPIO12 for  LED blue
...
//LV 9/Dic/23
/* An HTTP GET handler */
esp_err_t ledRGB_handler(httpd_req_t *req) {
    char* buf;
    size_t buf_len;
    ...
    if (buf_len > 1) {
        ...
        if (httpd_req_get_url_query_str(req, buf, buf_len) == ESP_OK) {
            ...
            if (httpd_query_key_value(buf, "ledRed", param1, sizeof(param1)) == ESP_OK &&
                httpd_query_key_value(buf, "ledGreen", param2, sizeof(param2)) == ESP_OK &&
                httpd_query_key_value(buf, "ledBlue", param3, sizeof(param3)) == ESP_OK) {
                
                ledRed = atoi(param1);
                ledGreen = atoi(param2);
                ledBlue = atoi(param3);

                httpd_resp_sendstr(req, "Variables set successfully");
                ...
                httpd_resp_sendstr(req, "Invalid parameters");
            }
        } else {
            httpd_resp_send_500(req);
        }
...
static esp_err_t status_handler(httpd_req_t *req)
{
    ...
    *p++ = '{';
    ...
    //LV 9/Dic/23
    p+=sprintf(p, "\"ledRed\":%d,", ledRed);
    p+=sprintf(p, "\"ledGreen\":%d,", ledGreen);
    p+=sprintf(p, "\"ledBlue\":%d,", ledBlue);
...
//LV 9/Dic/23
    httpd_uri_t ledRGB_uri = {
        .uri       = "/ledRGB",
        .method    = HTTP_GET,
        .handler   = ledRGB_handler,
        .user_ctx  = NULL
    };
    ...
    if (httpd_start(&camera_httpd, &config) == ESP_OK)
    {
        ...
        //LV 9/Dic/23
        httpd_register_uri_handler(camera_httpd, &ledRGB_uri);
